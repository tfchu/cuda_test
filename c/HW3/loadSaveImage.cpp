...
{
  cv::Mat image = cv::imread(filename.c_str(), IMREAD_COLOR | IMREAD_ANYDEPTH); // CV_LOAD_IMAGE_COLOR, CV_LOAD_IMAGE_ANYDEPTH
  if (image.empty()) {
...
void loadImageRGBA(const std::string &filename,
                   ...
                   size_t *numRows, size_t *numCols)
{
  cv::Mat image = cv::imread(filename.c_str(), IMREAD_COLOR);    // CV_LOAD_IMAGE_COLOR
  if (image.empty()) {
    std::cerr << "Couldn't open file: " << filename << std::endl;
    exit(1);
  }
  ...
  cv::Mat imageRGBA;
  cv::cvtColor(image, imageRGBA, cv::COLOR_BGR2RGBA);    // CV_BGR2RGBA
                   ...
                   const std::string &output_file)
{
  ...
  cv::cvtColor(imageRGBA, imageOutputBGR, cv::COLOR_RGBA2BGR);  // CV_RGBA2BGR
  //output the image
